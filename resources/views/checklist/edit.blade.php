        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('checklist.update', $checklist) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <x-textarea id="vehicle_condition" class="block mt-1 w-full" name="vehicle_condition">
                                {{ old('vehicle_condition', $checklist->vehicle_condition) }}
                            </x-textarea>

                            <x-input-label for="notes" :value="__('Notes')" />
                            <x-textarea id="notes" class="block mt-1 w-full" name="notes">
                                {{ old('notes', $checklist->notes) }}
                            </x-textarea>

                            {{-- <x-input-label for="status" :value="__('Status')" />
                            <x-select name="status" id="status">
                                <option value="draft"
                                    {{ old('status', $checklist->status) === 'draft' ? 'selected' : '' }}>Draft
                                </option>
                            </x-select> --}}
                        </div>
